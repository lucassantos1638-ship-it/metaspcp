-- Fix negative time values in existing data
UPDATE producoes SET minutos_normais = 0 WHERE minutos_normais < 0;
UPDATE producoes SET minutos_extras = 0 WHERE minutos_extras < 0;

-- Create or Replace Function to Calculate Overtime (FIXED)
...
BEGIN
    ...
    -- Calculate total duration (FIX: Ensure non-negative)
    v_minutos_totais := GREATEST(0, FLOOR(EXTRACT(EPOCH FROM (v_data_fim - v_data_inicio)) / 60));

    -- If no schedule, everything is normal (or simple calc)
    IF v_escala_semanal IS NULL THEN
        NEW.minutos_normais := v_minutos_totais;
