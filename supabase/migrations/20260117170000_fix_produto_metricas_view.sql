...
custos_producao AS (
    SELECT 
        ...
        p.etapa_id,
        p.subetapa_id,
        (EXTRACT(EPOCH FROM (p.hora_fim - p.hora_inicio)) / 3600.0) * COALESCE(c.custo_hora, 0) as custo_total_entrada,
        (EXTRACT(EPOCH FROM (p.hora_fim - p.hora_inicio)) / 60.0) as minutos_totais
    FROM producoes p
    LEFT JOIN colaboradores c ON p.colaborador_id = c.id
    WHERE p.data_fim IS NOT NULL -- Only finished productions contribute to history
),
metricas_agregadas AS (
    SELECT
        ...
        SUM(ul.quantidade_total) as soma_qtd_lotes, -- Total items produced in these lots
