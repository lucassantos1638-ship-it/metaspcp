...
export default function ProjecaoMensal() {
    ...
    const resumoProdutos = useMemo(() => {
        ...
        listaFiltrada.forEach((proj: any) => {
            proj.itens?.forEach((item: any) => {
    ...
    const handleAbrirDetalhes = (projecao: any) => {
        setProjecaoSelecionada(projecao);
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-4rem)] -m-8 p-8 overflow-hidden gap-6 bg-background">
            ...
            <div className="flex items-center gap-4 shrink-0 no-print">
                <Button variant="ghost" size="icon" onClick={() => navigate(`/projecao-vendas/${usuarioId}`)}>
                    <ArrowLeft className="h-5 w-5" />
                </Button>
                <div>
                    <h2 className="text-3xl font-bold tracking-tight">{format(dataMes, "MMMM yyyy", { locale: ptBR })}</h2>
                    <p className="text-muted-foreground">Resumo de {usuarioNome}</p>
                </div>
            </div>
            ...
            >
                <div className="hidden print:block p-4 mb-2 text-center border-b">
                    <h1 className="text-xl font-bold uppercase">{format(dataMes, "MMMM yyyy", { locale: ptBR })}</h1>
                ...
                <div className="p-3 border-b bg-muted/20 font-semibold text-sm flex justify-between items-center shrink-0 print:hidden">
                    <span>Resumo Geral de Produtos</span>
                        ...
                        <TableBody>
                            {resumoProdutos.map((item: any) => (
                                <TableRow key={item.sku || item.produto}>
                                    ...
                                    <TableCell className="text-right font-bold">{formatarCusto(item.valorTotal)}</TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
                    </Table>
                ...
                <div className="p-3 border-t bg-muted/20 flex justify-between items-center text-sm font-bold shrink-0">
                    ...
                    <div className="flex gap-8">
                        ...
                        <span className="text-primary text-base">Total: {formatarCusto(totaisGerais.valorTotal)}</span>
                    </div>
                </div>
            </div>

            {/* Bottom Section: Projections List */}
            <div className={`basis-1/2 overflow-hidden flex flex-col gap-2 no-print ${expandido ? 'hidden' : ''}`}>
                <h3 className="font-semibold text-lg shrink-0">Projeções Individuais</h3>
                <ScrollArea className="flex-1 border rounded-lg bg-card p-0">
                    <div className="flex flex-col">
                        {listaFiltrada.map((proj: any) => {
                            const projTotal = proj.itens?.reduce((acc: number, item: any) => acc + (item.quantidade * item.valor_unitario), 0) || 0;
                            const projItens = proj.itens?.reduce((acc: number, item: any) => acc + item.quantidade, 0) || 0;
                            ...
                            return (
                                <div
                                    ...
                                    onClick={() => handleAbrirDetalhes(proj)}
                                >
                                    <div className="flex flex-col gap-1">
                                        <div className="flex items-center gap-2">
                                            <span className="font-semibold">{proj.cliente_nome}</span>
                                    ...
                                    <div className="flex items-center gap-4">
                                        <div className="text-right">
                                            <div className="font-bold text-base">{formatarCusto(projTotal)}</div>
                                        </div>
