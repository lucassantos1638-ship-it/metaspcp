...
export default function ProjecaoUsuario() {
    ...
    const listaUsuario = useMemo(() => {
        return projecoes?.filter((p: any) => p.usuario_id === usuarioId) || [];
    }, [projecoes, usuarioId]);
    ...
    const meses = useMemo(() => {
        const groups: any = {};
        listaUsuario.forEach((proj: any) => {
            const date = parseISO(proj.data_referencia);
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-7xl mx-auto">
            <div className="flex items-center gap-4">
                <Button variant="ghost" size="icon" onClick={() => navigate("/projecao-vendas")}>
            ...
            <div className="flex flex-col gap-2">
                {meses.map((mes: any) => (
                    <div
                        ...
                        onClick={() => navigate(`/projecao-vendas/${usuarioId}/${mes.key}`)}
                    >
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground mt-1">
                                    {mes.count} {mes.count === 1 ? 'projeção' : 'projeções'}
                                </p>
                            </div>
                        </div>
                ...
                {meses.length === 0 && (
                    <div className="py-10 text-center text-muted-foreground">
                        Nenhuma projeção encontrada para este usuário.
                    </div>
                )}
            </div>
        </div>
