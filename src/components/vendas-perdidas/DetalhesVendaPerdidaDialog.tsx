...
import { useAdicionarItemVendaPerdida, useAtualizarItemVendaPerdida, useRemoverItemVendaPerdida, useVendaPerdida } from "@/hooks/useVendasPerdidas";
...
interface DetalhesVendaPerdidaDialogProps {
    open: boolean;
    onOpenChange: (open: boolean) => void;
    venda: any;
}

export default function DetalhesVendaPerdidaDialog({
    open,
    onOpenChange,
    venda: vendaInicial,
}: DetalhesVendaPerdidaDialogProps) {
    // Determine ID from initial object
    const id = vendaInicial?.id;

    // Fetch fresh data
    const { data: vendaData, isLoading: isLoadingVenda } = useVendaPerdida(id);

    // Use fetched data if available, otherwise initial (or null)
    const venda = vendaData || vendaInicial;

    const [adicionando, setAdicionando] = useState(false);
    const adicionarItem = useAdicionarItemVendaPerdida();
    const removerItem = useRemoverItemVendaPerdida();
    const atualizarItem = useAtualizarItemVendaPerdida();
    ...
    const totalValor = venda.itens?.reduce((acc: number, item: any) => acc + (Number(item.quantidade) * Number(item.valor_unitario)), 0) || 0;
    const totalQtde = venda.itens?.reduce((acc: number, item: any) => acc + Number(item.quantidade), 0) || 0;
                ...
                <DialogHeader className="p-6 border-b">
                    <div className="flex justify-between items-start">
                        <div className="space-y-1">
                            <DialogTitle className="text-2xl font-bold">{venda.cliente_nome}</DialogTitle>
                            <div className="flex items-center gap-2 text-base text-muted-foreground">
                                <span>{venda.data_referencia ? format(parseISO(venda.data_referencia), "dd 'de' MMMM 'de' yyyy", { locale: ptBR }) : 'Sem data'}</span>
                            </div>
                        </div>
                        <div className="text-right">
                            <div className="text-sm text-muted-foreground">Total Perdido Estimado</div>
                            <div className="text-2xl font-bold text-destructive">{formatarCusto(totalValor)}</div>
                                            ...
                                            {produtosFiltrados.map(p => (
                                                ...
                                                >
                                                    ...
                                                    <span className="font-semibold text-primary">
                                                        {formatarCusto(venda.tabela_preco === 'cnpj' ? p.preco_cnpj : p.preco_cpf)}
                                                    </span>
                                                </div>
                                            ))}
                                    ...
                                    {(!venda.itens || venda.itens.length === 0) && (
                                        <TableRow>
                                            <TableCell colSpan={6} className="text-center py-8 text-muted-foreground">
                                                Nenhum item adicionado à venda perdida.
                                            </TableCell>
                                        </TableRow>
                                    )}
                                </TableBody>
